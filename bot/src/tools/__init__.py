...
async def send_large_message(text, parse_mode='markdown'):
    try:
        if len(text) <= 4090:
            create_task(bot.send_message(message=f'```{text}```', entity=channel, reply_to=thread, link_preview=False, parse_mode=parse_mode))
        else:
            ...
            for part in message_parts:
                for _ in range(0, len(message_parts)):
                    create_task(bot.send_message(message=f'```{part}```', entity=channel, reply_to=thread, link_preview=False, parse_mode=parse_mode))
    except Exception as e:
        logger.debug(str(e))
