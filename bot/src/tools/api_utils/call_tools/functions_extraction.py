...
def get_openai_funcs(return_function_objects = None):
    route = "bot/src/tools/api_utils/call_tools/tools_frontend"
    module_route = route.replace("/", ".")
    function_map = import_functions_from_directory(route, module_route)
...
def extract_function_info(func: Callable) -> dict:
    ...
    c.tools_loaded.append(func.__name__)
    return spec
