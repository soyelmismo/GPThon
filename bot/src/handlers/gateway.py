...
from bot.src.tools.tg_tools import get_id, is_bot_mentioned, whitelist_check
...
async def gateway(event) -> None:
    ...
    if command and callingTo:
        ...
        return create_task(callingTo(event, user_id = user_id, chat_id=chat_id, command = command))
